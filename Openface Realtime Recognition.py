...
employee_pictures = "Face_recognition_samples/"

employees = dict()

for file in listdir(employee_pictures):
	employee, extension = file.split(".")
	img = preprocess_image('Face_recognition_samples/%s.jpg' % (employee))
...
while(True):
	...
	for (x1, y1, x2, y2) in faces:
		...
		if w > 35: #discard small detected faces
			cv2.rectangle(img, (x,y), (x+w,y+h), color, 2) #draw rectangle to main image
			...
			for i in employees:
				...
				if index == np.argmin(distances):
					if distances[index] <= threshold:
						...
						if similarity > 105: similarity = 90
